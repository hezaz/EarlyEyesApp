            ...
            <tr>
                ...
                <td>
                     @if(!empty($user->parent))
                         @if(!empty($user->parent->userDetails))
                            @if($user->parent->userDetails->profile_pic != '')
                                @php $url =  $_ENV['APP_URL'].'public/images/user/'.$user->parent->userDetails->profile_pic; @endphp
                            @else
                                @php $url = $_ENV['APP_URL'].'public/admin/dist/img/default-150x150.png'; @endphp
                            @endif
                        @else
                     ...
                     @else
                        @php $url = $_ENV['APP_URL'].'public/admin/dist/img/default-150x150.png'; @endphp
                    @endif
                     @if(!empty($user->parent))<img src="{{$url}}" alt="" class="img-circle img-size-32 mr-2"  style="width: 32px; height: 32px;">@endif
                 @if(!empty($user->parent)) {{ucwords($user->parent->full_name)}}@endif
                </td>
                 <td>{{$user->email}}</td>
                <td>@if(!empty($user->parent)){{$user->parent->mobile_number}}@endif</td>
                <td>{{$user->userDetails->class}}</td>
                <td>{{$user->userDetails->course_name}}</td>
                ...
                <td>
                     <button class="btn btn-success parent">Student</button>
                </td>
                <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                       <a class="dropdown-item text-info" href="{{url('admin/users/info/'.$user->id.'/student')}}">Info</a>
                        <a class="dropdown-item text-warning" href="{{url('admin/users/unblock/'.$user->id.'/student')}}">Unblock</a>
