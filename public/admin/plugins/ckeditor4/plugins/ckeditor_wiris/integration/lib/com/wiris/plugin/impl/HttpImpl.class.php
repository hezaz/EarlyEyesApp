...
function com_wiris_plugin_impl_HttpImpl_0(&$ÂÂ»this, &$listener, &$url, $status) {
	{
	}
}
function com_wiris_plugin_impl_HttpImpl_1(&$ÂÂ»this, &$listener, &$url, $msg) {
	{
		if($ÂÂ»this->listener !== null) {
			$ÂÂ»this->listener->onError($msg);
		} else {
			throw new HException($msg);
		}
	}
}
function com_wiris_plugin_impl_HttpImpl_2(&$ÂÂ»this, &$listener, &$url, $data) {
	{
		$ÂÂ»this->data = $data;
		if($ÂÂ»this->listener !== null) {
			$ÂÂ»this->listener->onData($data);
		}
	}
}
