...
function haxe_Http_0(&$ÂÂ»this, &$url, $status) {
	{
	}
}
function haxe_Http_1(&$ÂÂ»this, &$url, $msg) {
	{
	}
}
function haxe_Http_2(&$ÂÂ»this, &$url, $data) {
	{
	}
}
function haxe_Http_3(&$ÂÂ»this, &$_g, &$a, &$api, &$b, &$chunked, &$headers, &$hline, &$hname, &$k, &$response, &$rp, &$s, &$size, &$sock, &$status) {
	if($a->length === 1) {
		return $a[0];
	} else {
		return $a->join(": ");
...
function haxe_Http_4(&$ÂÂ»this, &$api, &$b, &$boundary, &$buf, &$bufsize, &$data, &$host, &$method, &$multipart, &$port, &$portString, &$post, &$request, &$secure, &$sock, &$uri, &$url_regexp) {
	if($ÂÂ»this->file->size > $bufsize) {
		return $bufsize;
	} else {
		return $ÂÂ»this->file->size;
	}
}
