...
class com_wiris_plugin_impl_TestImpl implements com_wiris_plugin_api_Test{
	...
	public function getTestPage() {
		...
		try {
			...
			$solutionLink = "";
			$condition = true;
		}catch(Exception $ÂÂ»e) {
			$_ex_ = ($ÂÂ»e instanceof HException) ? $ÂÂ»e->e : $ÂÂ»e;
			$ex = $_ex_;
			{
...
function com_wiris_plugin_impl_TestImpl_0(&$ÂÂ»this, &$condition, &$ex, &$imageUrl, &$mml, &$outp, &$output, &$p, &$param, &$platform, &$provider, &$random, &$reportText, &$solutionLink, &$testName) {
	{
...
function com_wiris_plugin_impl_TestImpl_1(&$ÂÂ»this, &$condition, &$ex, &$imageUrl, &$mml, &$outp, &$output, &$p, &$param, &$platform, &$provider, &$random, &$reportText, &$s2, &$solutionLink, &$testName) {
	{
		$s = new haxe_Utf8(null);
		$s->addChar(187);
		return $s->toString();
...
function com_wiris_plugin_impl_TestImpl_2(&$ÂÂ»this, &$condition, &$ex, &$imageUrl, &$mml, &$outp, &$output, &$p, &$param, &$platform, &$provider, &$random, &$reportText, &$s2, &$solutionLink, &$testName) {
	{
		$s = new haxe_Utf8(null);
		$s->addChar(168);
		return $s->toString();
